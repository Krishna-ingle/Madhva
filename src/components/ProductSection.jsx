...
const ProductSection = () => {
  ...
  const checkScrollPosition = () => {
    if (scrollContainerRef.current) {
      ...
      // (Math.ceil helps prevent rounding errors on high-DPI screens)
      setCanScrollRight(Math.ceil(scrollLeft + clientWidth) < scrollWidth);
    }
  };

  // Run the check on initial load and whenever window resizes
  useEffect(() => {
    checkScrollPosition();
    ...
    <section  id="product" className="py-16 bg-white w-full overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          {/* Left Scroll Fade & Button - Conditionally Rendered */}
          {canScrollLeft && (
          ...
          {/* Scroll Container with onScroll event listener attached */}
          <div 
            ref={scrollContainerRef} 
            onScroll={checkScrollPosition}
            className="flex overflow-x-auto gap-4 pb-8 snap-x snap-mandatory hide-scrollbar relative z-0"
          >
            {productData.map((product) => (
              ...
              >
                ...
                <div className="w-full h-50 mt-auto flex items-end justify-center">
                  <img
                    src={hoveredCard === product.id ? product.gifSrc : product.staticSrc}
          ...
          {/* Right Scroll Fade & Button - Conditionally Rendered */}
          {canScrollRight && (
